...
ig.GameFlow = ig.Class.extend({
	...
	startNewGame: function (params) {
		...
		if(params.level == 'sandbox'){
			...
			ig.game.spawnEntity( EntityNpc_bird, 0, 0, { start:'left' });
		}
		if(params.hero === 0){
			ig.game.spawnEntity( EntityStrongman, 100, 20, { buttons: buttons, hero:params.hero });
		}
		if(params.hero === 1){
        	ig.game.spawnEntity( EntityCthulhu, 100, 20, { buttons: buttons, hero:params.hero });
        }
	},
	...
	nextLevel: function(params){
        ...
        if(inHand){
 		...
 		if(hero === 0){
			ig.game.spawnEntity( EntityStrongman, 100, 20, { buttons: buttons, hero:hero });
		}
		if(hero === 1){
        	ig.game.spawnEntity( EntityCthulhu, 100, 20, { buttons: buttons, hero:hero });
        }
	},
