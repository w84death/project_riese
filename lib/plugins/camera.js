...
ig.Camera = ig.Class.extend({
    ...
    shake: function(time){
        var shake = time || this.shakeTime;
        if(ig.game.enableShake){
            this.shakeTimer.set(shake);
        }
    },
    ...
    update: function() {
        ...
        if( players[0] ) {
            ...
            this.sY = this.k * this.sY + (1.0 - this.k) * (players[0].pos.y);

        }

        if(this.shakeTimer.delta() < 0){
            if(Math.random()*10 > 5){
                shakeX = 4 - ((Math.random()*8)<<0);
                shakeY = 4 - ((Math.random()*8)<<0);
            }
        }
