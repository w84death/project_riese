...
.defines(function(){
	...
	EntityWeapon = ig.Entity.extend({
		...
		drop: function( other ){
            var center = other.pos.x + (other.size.x*0.5);
			if(other.flip){
				this.pos.x = center + 24;
			}else{
				this.pos.x = center - 24 - this.size.x;
			}
            this.accel.y -= 56;
			other.inHand = false;
            this.owner = false;
			...
			if(this.ammo > 0){
				if(this.shootTimer.delta() > this.shootTimerDelay){
					...
					if(!this.owner.flip){
                        ...
                        this.owner.push(this.feedbackPower);
					}else{
						ig.game.spawnEntity(
							EntityBullet,
							this.pos.x+this.offset.x + 4,
		...
		update: function() {
			...
			if(this.owner){
                ...
                var center = this.owner.pos.x + this.owner.offset.x;

				if(!this.owner.flip){
					this.currentAnim.flip.x = false;
                    this.pos.x = center + this.owner.weaponPosition.x - this.owner.size.x - (this.size.x*0.5)<<0;
				}else{
					this.currentAnim.flip.x = true;
                    this.pos.x = center + this.owner.weaponPosition.x;
				}
			}else{
				this.currentAnim = this.anims.idle;
			}
